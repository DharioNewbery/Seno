...
- **Interface web:** SvelteKit (Svelte 5 + TypeScript + Vite), CodeMirror 6; código em `../Seno-ui` (diretório irmão do repo), build estática embutida aqui via `go:embed`
...
web/                       # Saída do build da UI (web/static, gerada pelo adapter-static) + embed em web.go (SPA fallback)

Interface web (fora deste repo, diretório irmão):
../Seno-ui/
  src/lib/api/             # Cliente fetch (envelope, Bearer, refresh) + tipos da API
  src/lib/stores/          # Sessão reativa (localStorage: seno.access/refresh/user)
  src/lib/components/      # Componentes Svelte (CodeEditor etc.)
  src/routes/              # Páginas: /, professors, classes, assignments, assignments/[id]
  app.css                  # Design tokens + componentes (ver Seno-ui/STYLEGUIDE.md)
```
...
Interface web (SvelteKit, em `../Seno-ui`):

```bash
# Dependências (primeira vez)
cd ../Seno-ui && npm install

# Build da UI → escreve a saída em ../Seno/web/static (deve rodar antes de go build)
npm run build

# Dev server com proxy /api e /health → http://localhost:8080 (API Go rodando)
npm run dev
